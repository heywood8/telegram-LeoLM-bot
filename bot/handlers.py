...
class BotHandlers:
    ...
    async def _handle_system_prompt_update(
        self,
        message,
        user,
        chat_id: int,
        ...
        Returns:
            tuple: (was_handled, response_message)
                   was_handled: True if this was a system prompt update
                   response_message: Response to send to user (None if success)
        """
        # Check if this is a system prompt update from an admin
        if chat_id not in self._waiting_for_prompt:
            return False, None
    ...
    def _clean_message_text(self, message_text: str) -> str:
        """Clean message text by removing assistant role markers.
    ...
    def _check_group_chat_addressing(
        self,
        message,
        message_text: str,
        context: ContextTypes.DEFAULT_TYPE
    ) -> tuple[bool, str]:
        """Check if bot is addressed in group chat and clean mention.
        ...
        Returns:
            tuple: (is_addressed, cleaned_text)
                   is_addressed: True if bot was addressed or should respond
                   cleaned_text: Message text with mentions removed
        """
        ...
        return _is_addressed_in_group(message_text)

    async def _check_rate_limit(self, update: Update, user) -> tuple[bool, str | None]:
        ...
        Returns:
            tuple: (should_continue, error_message)
                   should_continue: True if message should be processed
                   error_message: Error message to send if rate limited
        """
        # Check rate limit
        allowed, retry_after = await self.rate_limiter.check_limit(user.id)
        if not allowed:
            error_msg = f"⏱️ Rate limit exceeded. Please try again in {retry_after} seconds."
    ...
    async def _send_response(
        ...
        Args:
            ...
            websearch_called: Whether web search was called
        """
    ...
    async def _process_llm_request(
        ...
        """Process LLM request with tools and return response.

        Args:
            user_session: User session object
            message_text: Cleaned message text
            message: Telegram message object
            user: Telegram user object
            db: Database session

        Returns:
            tuple: (response_text, tool_called, websearch_called)
        """
        ...
        context_messages = await self.session_manager.get_context_window(
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            async with async_session_factory() as db:
                ...
                response_text, tool_called, websearch_called = await self._process_llm_request(
